...
function Sidebar() {
  ...
  const fetchGroups = async () => {
    ...
    const userId = localStorage.getItem("userId"); // 유저 ID 저장 필요

    try {
      const res = await fetch(`https://calpickbe-production.up.railway.app/api/groups/${userId}`, {
        headers: {
...
{isGroupModalOpen && <GroupCreateModal onClose={closeGroupModal} />}
  </>
  );
}
